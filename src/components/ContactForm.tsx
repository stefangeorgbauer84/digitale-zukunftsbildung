...
export default function ContactForm({ light = false }: { light?: boolean }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label htmlFor="anliegen" className={labelCls}>Worum geht es?</label>
      ...
      <button type="submit" disabled={status === 'loading'}
        className={`w-full py-3.5 rounded-xl text-sm font-body font-700 transition-all active:scale-95 disabled:opacity-60 ${
          light
      ...
      {status === 'success' && (
        <div className={`rounded-xl p-4 text-sm font-body font-600 flex items-center gap-2 ${light ? 'bg-white/15 text-emerald-300' : 'bg-status-teal-light text-status-teal'}`}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
          Danke! Wir melden uns innerhalb von 24 Stunden.
        </div>
      )}
      {status === 'error' && (
        <div className={`rounded-xl p-4 text-sm font-body font-600 flex items-center gap-2 ${light ? 'bg-white/15 text-red-300' : 'bg-red-50 text-red-600'}`}>
