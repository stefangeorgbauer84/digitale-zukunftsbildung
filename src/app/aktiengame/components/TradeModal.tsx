...
interface TradeModalProps {
  ...
  onBuy: (qty: number, confidence: number) => void
  onSell: (qty: number, confidence: number) => void
  onClose: () => void
}
          ...
          <div className="p-6 space-y-4">
            <div className="text-center">
              ...
              <h3 className="font-heading text-lg font-bold text-primary-dark mb-1">
                Wie sicher bist du mit dieser Entscheidung?
              </h3>
              <p className="text-sm text-text-muted">
                {pendingAction === 'buy' ? 'Kauf' : 'Verkauf'} von {qty} × {asset.name}
              </p>
            </div>
            <div className="flex gap-3 justify-center mt-4">
              {[
                { rating: 1, label: 'Unsicher', emoji: '😬' },
                { rating: 2, label: 'Okay', emoji: '🙂' },
                { rating: 3, label: 'Sehr sicher', emoji: '💪' },
              ].map(({ rating, label, emoji }) => (
                <button
                  key={rating}
                  onClick={() => handleConfidence(rating)}
                  className="flex flex-col items-center gap-1 border-2 border-gray-200 hover:border-primary-medium rounded-xl px-4 py-3 transition-all hover:bg-primary-50 group"
                >
                  <span className="text-2xl">{emoji}</span>
                  <span className="text-xs font-semibold text-text-secondary group-hover:text-primary-dark">{label}</span>
                </button>
              ))}
            </div>
            <button
              onClick={() => setShowConfidence(false)}
              className="w-full text-xs text-text-muted underline mt-2"
            >
              Abbrechen
            </button>
          </div>
                ...
                <div className="flex gap-2 mt-2">
                  {tab === 'buy' && (
                    <button
                      ...
                      className="flex-1 text-xs font-semibold border border-status-teal text-status-teal rounded-lg py-1.5 hover:bg-status-teal-light transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      Max. kaufen ({maxBuyQty} Stk.)
                    </button>
                  )}
                  {tab === 'sell' && (
                    <button
                      onClick={() => maxSellQty > 0 && setQty(maxSellQty)}
                      disabled={maxSellQty <= 0}
                      className="flex-1 text-xs font-semibold border border-red-400 text-red-500 rounded-lg py-1.5 hover:bg-red-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      Alle verkaufen ({maxSellQty} Stk.)
                    </button>
                  )}
                </div>
              </div>
